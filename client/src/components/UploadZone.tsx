...
const UploadZone = ({label,file,onClear,onChange}: UploadZoneProps) => {
  return (
    <div className="relative group">
      <div className={`relative h-64 rounded-2xll border-2 border-dashed transition-all duration-300 flex flex-xol items-center justify-center bg-white/2 p-6 ${file ? 'border-violet-600/50 bg-violet-500/5' : 'border-white/10 hover:border-violet-500/30 hover:bg-white/5'}`}>
        {file ? (
            <>
            <img src={URL.createObjectURL(file)} alt="Preview" className="absolute inset-0 w-full object-cover rounded-xl opacity-60" />
            <div>
            ...
            <input className="absolute inset-0 w-full h-full opacity=0 cursor-pointer" type="file" accept="image/*" onChange={onChange} />
            </>
        )}
      </div>
    </div>
